...
```{r}
s_teams <- filter(Teams, yearID >= 1990, yearID <= 2015 )
s_salaries <- filter(Salaries, yearID >= 1990, yearID <= 2015)
```
...
by_teams_year <- s_salaries %>%
  group_by(teamID, yearID) %>%
  summarize(
    payroll = sum(salary, na.rm = TRUE) 
  )
by_teams_year
...
```{r}
joined <- left_join(s_teams, by_teams_year, by = NULL)
joined
```
...
> ANS:  The Oakland A's are more efficient than the other teams and was most efficient in around 2000. 
